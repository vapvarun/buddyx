...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function register_blocks(): void {
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			...
			do_action( 'buddyx_log', '[BuddyX Blocks] init registrar at ' . $blocks_dir );
		}
		if ( ! is_dir( $blocks_dir ) ) {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				// Log missing blocks directory.
				do_action( 'buddyx_log', '[BuddyX Blocks] blocks dir missing' );
			}
			return;
		}
		...
		if ( empty( $dirs ) ) {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				...
				do_action( 'buddyx_log', '[BuddyX Blocks] no block directories found' );
			}
			return;
		}
		...
		foreach ( $dirs as $dir ) {
			...
			if ( ! file_exists( $block_json ) ) {
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					...
					do_action( 'buddyx_log', '[BuddyX Blocks] skipping (no block.json): ' . $dir );
				}
				continue;
			}
			...
			try {
				...
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					...
					do_action( 'buddyx_log', '[BuddyX Blocks] registered: ' . $dir );
				}
			} catch ( \Throwable $e ) {
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					// Log block registration failure.
					do_action( 'buddyx_log', '[BuddyX Blocks] failed to register ' . $dir . ' :: ' . $e->getMessage() );
				}
			}
		}
	}
