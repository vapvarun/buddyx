 ...
 *
 * The contextual filter is of the form `buddyx_attr_{context}_output`.
 *
 * @since 1.0.0
 *
 ...
 */
function buddyx_attr( $context, $attributes = array(), $args = array() ) {

	$attributes = buddyx_parse_attr( $context, $attributes, $args );
	...
	$output = apply_filters( "buddyx_attr_{$context}_output", $output, $attributes, $context, $args );
 ...
 *
 * The contextual filter is of the form `buddyx_attr_{context}`.
 *
 * @since 1.0.0
 *
 * @param string $context    The context, to build filter name.
...
function buddyx_parse_attr( $context, $attributes = array(), $args = array() ) {
	...
	// Contextual filter.
	return apply_filters( "buddyx_attr_{$context}", $attributes, $context, $args );
}
...
function buddyx_get_breadcrumb( $echo = true ) {
	...
	$defaults = array(
		'before'      => '<div class="buddyx-breadcrumbs">',
		'after'       => '</div>',
		'show_browse' => false,
		'echo'        => $echo,
	);

	$args = apply_filters( 'buddyx_breadcrumb_trail_args', $defaults );

	$breadcrumb = apply_filters( 'buddyx_breadcrumb_trail_object', null, $args );

	if ( ! is_object( $breadcrumb ) ) {
		$breadcrumb = new BuddyX_Breadcrumbs( $args );
	}
...
class BuddyX_Breadcrumbs {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'buddyx_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= sprintf(
				'<%1$s class="buddyx-trail-items" %2$s>',
			...
			if ( $this->args['schema'] ) {
				...
				$breadcrumb .= sprintf(
					'<meta content="%1$d" %2$s />',
					absint( $item_count ),
					buddyx_attr(
						'breadcrumb-buddyx-trail-items-num-meta',
						array(
							'name'  => 'numberOfItems',
							'class' => '',
						)
				...
				$breadcrumb .= '<meta ' . buddyx_attr(
					'breadcrumb-buddyx-trail-items-list-meta',
					array(
						'class'   => '',
						'name'    => 'itemListOrder',
						'content' => 'Ascending',
		...
		$breadcrumb = apply_filters( 'buddyx_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:', 'buddyx' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'buddyx' ),
			'home'                => esc_html__( 'Home', 'buddyx' ),
			'error_404'           => esc_html__( '404 Not Found', 'buddyx' ),
			'archives'            => esc_html__( 'Archives', 'buddyx' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s', 'buddyx' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'buddyx' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s', 'buddyx' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'buddyx' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'buddyx' ),
			...
			'bp_register'         => esc_html__( 'Register', 'buddyx' ),
		);

		$this->labels = apply_filters( 'buddyx_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'buddyx_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		// If not viewing the front page.
			...
			// If viewing the register page.
			elseif ( class_exists( 'BuddyPress' ) && bp_is_register_page() ) {
				$this->add_register_items();
			}
		...
		$this->items = array_unique( apply_filters( 'buddyx_breadcrumb_trail_items', $this->items, $this->args ) );
	}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'buddyx' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'buddyx' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'buddyx' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'buddyx' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'buddyx' ) ) );
		$day   = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'buddyx' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'buddyx' ) ) );
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'buddyx' ) ) );
	 ...
	 * @return void
	 */
	protected function add_register_items() {

		if ( true === $this->args['show_title'] ) {
			$this->items[] = $this->labels['bp_register'];
		}
	}
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'buddyx' ) ) ) );
				}

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'buddyx' ) ) ) );
				}

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'buddyx' ) ) ) );
				}
