					...
					<div class="wb-bp-user-avatar-wrap">
						<div class="wb-bp-user-avatar">
							...
							if ($current_user instanceof WP_User) {
								$user_link = '#'; // Default link.

								if (function_exists('buddypress')) {
									// Check BuddyPress version.
									if (version_compare(buddypress()->version, '12.0.0', '>=')) {
										// If BuddyPress version is 12.0.0 or higher, use bp_members_get_user_url().
										if (function_exists('bp_members_get_user_url')) {
											$user_link = bp_members_get_user_url(get_current_user_id());
										}
									} else {
										// For older versions of BuddyPress, use bp_core_get_user_domain().
	...
	?>
		...
		<ul>
			...
			?>
				<li>
					<label>
						<input class="widefat" type="checkbox" name="<?php echo esc_attr($this->get_field_name('profile_groups_enabled')); ?>[]" value="<?php echo esc_attr($single_group_details->id); ?>" <?php checked($is_checked); ?> />
