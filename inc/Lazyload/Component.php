<?php
/**
 * WbcomDesigns\WbcomDesigns\Lazyload\Component class
 *
 * @package buddyx
 */

namespace WbcomDesigns\WbcomDesigns\Lazyload;
...
class Component implements Component_Interface {
	...
	public function action_customize_register_lazyload( WP_Customize_Manager $wp_customize ) {
		...
		$wp_customize->add_control(
			'lazy_load_media',
			[
				'label'           => __( 'Lazy-load images', 'buddyx' ),
				'section'         => 'theme_options',
				'type'            => 'radio',
				'description'     => __( 'Lazy-loading images means images are loaded only when they are in view. Improves performance, but can result in content jumping around on slower connections.', 'buddyx' ),
	...
	public function action_enqueue_lazyload_assets() {
		wp_enqueue_script(
			'buddyx-lazy-load-images',
			get_theme_file_uri( '/assets/js/lazyload.min.js' ),
			[],
			buddyx()->get_asset_version( get_theme_file_path( '/assets/js/lazyload.min.js' ) ),
			false
		);
		wp_script_add_data( 'buddyx-lazy-load-images', 'defer', true );
		wp_script_add_data( 'buddyx-lazy-load-images', 'precache', true );
	}
