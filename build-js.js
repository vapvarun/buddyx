...
const SUPPORTED_EXT = ['.js', '.jsx', '.ts', '.tsx', '.cjs'];
...
files.forEach((file) => {
	...
	const outputPath = path.join(
		outDir,
		relativePath.replace(/\.(js|jsx|ts|tsx|cjs)$/, '.min.js')
	);
	const outputDir = path.dirname(outputPath);
	if (!existsSync(outputDir)) {
		mkdirSync(outputDir, { recursive: true });
	}
	const isCjs = path.extname(file) === '.cjs';
	esbuild
		.build({
			entryPoints: [file],
			outfile: outputPath,
			...
			bundle: !isCjs,
			target: ['es6'],
			loader: { '.js': 'jsx', '.jsx': 'jsx', '.ts': 'ts', '.tsx': 'tsx', '.cjs': 'js' },
