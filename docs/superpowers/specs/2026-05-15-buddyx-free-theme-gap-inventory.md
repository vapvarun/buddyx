...
| Custom font upload / management | Astra/Kadence/OceanWP let owners upload font files; the Astra *theme* itself relies on a companion **plugin** ("Custom Fonts" by Brainstorm Force), not theme code | **Out of scope for 5.1.0 — Customizer covers the real need.** BuddyX's Customizer typography stack (Inter default, 1,358 Google Fonts in a searchable grouped picker, "Load Google Fonts Locally" toggle for GDPR self-hosting, Kirki-era custom selections preserved, value-driven dynamic loader) gives owners what they actually configure. Classic-theme **custom font UPLOAD** is plugin territory — Astra ships it as a separate plugin too. WP 6.9's Font Library *is* technically reachable via `Appearance → Design → Styles → Typography → Manage fonts`, but the "Styles" landing button goes to the Style Book (not Typography), so the real path is a URL-deep-link or 4-5 clicks into nested panels, then a Google consent gate. End-to-end flow to the Customizer picker / front-end is unverified. **Not promoting that path in 5.1.0**: keeps UX consistent for 3,000 existing installs and avoids the hybrid-theme risk (flipping `wp_is_block_theme()` true would change template resolution + the Customizer menu). | — | No action in 5.1.0. Revisit later only if customers explicitly ask for theme-side font upload (then: build an upload UI feeding `available_font_families()` + the existing dynamic loader — the 5.1.0 plumbing makes that a Phase-2 lift, not a full build). |
...
Items 1-6 are single-commit S-effort and could plausibly land in one sprint.
Items 7-8 (M) are the natural follow-up. Per-entry display controls and
transparent header are tracked separately in their own design doc.
**Custom font upload** is out of scope (Customizer typography stack covers the
real need; theme-side upload UI is plugin territory — Astra ships it as a
separate plugin).
