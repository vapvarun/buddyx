...
| Custom font upload / management | Astra/Kadence/OceanWP let owners upload font files; the Astra *theme* itself does it via a companion **plugin** ("Custom Fonts"), not theme code | **Already available — verified in-browser.** WP 6.9's native Font Library (Library / Upload / Google Fonts tabs, plus BuddyX's `wp_register_font_collection()` "Modern Stacks" + "Local Fonts" tabs) is fully reachable on classic BuddyX *today*: Appearance -> Design -> Styles -> Typography -> "Manage fonts". WP gives this to any classic theme shipping a `theme.json` — BuddyX has one. `wp_is_block_theme()` stays `false`; front-end rendering is unaffected. The only real gap is **discoverability** — it lives in the Site Editor, not the Customizer where BuddyX owners work. | S | Add a "Manage custom fonts" link from the Customizer typography section to the Site Editor Font Library. Do **not** make the theme "hybrid" (`templates/index.html`) — that flips `wp_is_block_theme()` true, changing template resolution + the Customizer menu for all installs, for zero font gain. Astra (also classic + customizer + theme.json) does not go hybrid either. |
...
7. **Custom-font discoverability link** (S) — the WP Font Library (incl. Upload)
   already works on classic BuddyX via the Site Editor; just surface it with a
   link from the Customizer typography section. Do NOT make the theme hybrid.
8. **Related posts** (M) — needs a template part + grid styling + dark-mode pass, but high user value.
9. **Social share buttons** (M) — API-free share links + template part + position control.

Items 1-7 are single-commit S-effort and could plausibly land in one sprint.
Items 8-9 (M) are the natural follow-up. Per-entry display controls and
